...
import { Users, AlertTriangle, Award, IndianRupee, Heart, ChevronRight } from 'lucide-react';
...
const OwnerHome: React.FC = () => {
      ...
      <div className="min-h-screen bg-background pb-20">
        ...
        <div className="px-4 py-4 space-y-4">
          ...
          <div className="bg-card rounded-xl border border-border card-shadow p-4">
            <div className="font-display text-3xl font-extrabold text-foreground mb-1">₹{payrollLakhs}L</div>
            <div className="text-xs text-muted-foreground">{lang === 'hi' ? 'मासिक पेरोल' : 'Monthly Payroll'}</div>
          </div>
          <div className="bg-card rounded-xl border border-border card-shadow p-4">
            <div className="text-[10px] text-muted-foreground tracking-wider uppercase mb-3">{lang === 'hi' ? 'श्रेणी अनुसार विभाजन' : 'BY CATEGORY'}</div>
            {[
      ...
      <div className="min-h-screen bg-background pb-20">
        ...
        <div className="px-4 py-4 space-y-4">
          ...
          {lowScorers.map((p: any) => (
            <div key={p.id} className="bg-card rounded-xl border border-border card-shadow p-4 flex items-start gap-3">
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <TopBar />
      <div className="px-4 py-4 space-y-4">
        <div className="text-[10px] text-primary font-semibold tracking-[0.25em] uppercase">
        ...
        {topScorer && (
          <div className="bg-card rounded-xl border border-border card-shadow p-4 flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="font-display text-sm font-bold text-foreground">{topScorer.employees?.name} ({topScorer.employees?.emp_code})</div>
        ...
        <div className="space-y-2">
          <button onClick={() => setActiveTab('costs')} className="w-full flex items-center gap-4 p-4 rounded-xl border border-border bg-card card-shadow hover:bg-muted transition-colors">
          ...
          <button onClick={() => setActiveTab('people')} className="w-full flex items-center gap-4 p-4 rounded-xl border border-border bg-card card-shadow hover:bg-muted transition-colors">
