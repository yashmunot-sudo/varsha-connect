...
import { Users, Plus, FileText, ChevronRight } from 'lucide-react';
      ...
      <div className="min-h-screen bg-background pb-20">
        ...
        <div className="px-4 py-4 space-y-3">
          ...
          {teamMembers?.map((m) => {
            ...
            return (
              <div key={m.id} className="bg-card rounded-xl border border-border card-shadow p-4 flex items-center gap-3">
      ...
      <div className="min-h-screen bg-background pb-20">
        ...
        <div className="px-4 py-4 space-y-4">
          ...
          {casualWorkers?.map((cw: any) => (
            <div key={cw.id} className="bg-card rounded-xl border border-border card-shadow p-3 flex items-center gap-3">
              <div className="status-dot status-present" />
              <span className="text-sm text-foreground">{cw.name}</span>
              <span className="ml-auto text-[10px] text-muted-foreground">{cw.id_number || '-'}</span>
            </div>
          ))}
        </div>
        <BottomNav role="supervisor" activeTab={activeTab} onTabChange={setActiveTab} />
    ...
    return (
      <div className="min-h-screen bg-background pb-20">
        ...
        <div className="px-4 py-4 space-y-4">
          ...
          <div className="bg-card rounded-xl border border-border card-shadow p-4 space-y-4">
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">{lang === 'hi' ? 'उत्पादन संख्या' : 'Production Numbers'}</label>
              <input type="number" placeholder="0" className="w-full rounded-xl border border-border bg-background px-4 py-3 text-lg text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50" />
            </div>
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">{lang === 'hi' ? 'घटनाएं / नोट्स' : 'Incidents / Notes'}</label>
              <textarea rows={3} className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm text-foreground resize-none focus:outline-none focus:ring-2 focus:ring-primary/50" />
            </div>
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        <div className="space-y-2">
          <button onClick={() => setActiveTab('team')} className="w-full flex items-center gap-4 p-4 rounded-xl border border-border bg-card card-shadow hover:bg-muted transition-colors">
            <Users className="w-5 h-5 text-info" />
            <span className="text-sm font-semibold flex-1 text-left text-foreground">{lang === 'hi' ? 'टीम देखें' : 'View Team'}</span>
            <ChevronRight className="w-4 h-4 text-muted-foreground" />
          </button>
          <button onClick={() => setActiveTab('casual')} className="w-full flex items-center gap-4 p-4 rounded-xl border border-border bg-card card-shadow hover:bg-muted transition-colors">
            <Plus className="w-5 h-5 text-primary" />
            <span className="text-sm font-semibold flex-1 text-left text-foreground">{lang === 'hi' ? 'कैज़ुअल वर्कर लॉग' : 'Log Casual Workers'}</span>
            <ChevronRight className="w-4 h-4 text-muted-foreground" />
          </button>
          <button onClick={() => setActiveTab('report')} className="w-full flex items-center gap-4 p-4 rounded-xl border border-border bg-card card-shadow hover:bg-muted transition-colors">
            <FileText className="w-5 h-5 text-primary" />
            <span className="text-sm font-semibold flex-1 text-left text-foreground">{lang === 'hi' ? 'शिफ्ट रिपोर्ट' : 'Shift Report'}</span>
            <ChevronRight className="w-4 h-4 text-muted-foreground" />
          </button>
        </div>
      </div>
